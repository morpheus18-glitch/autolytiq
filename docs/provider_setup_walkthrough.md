...
This walkthrough aligns with the environment contract enforced by `backend/src/config/env.ts` and `ml_service/config/env.py`.
Complete each provider integration before running production workloads. Smoke commands should be executed from a secure
workstation or CI environment that already has the relevant SDK/CLI installed.
...
- The webhook signing key is required when `DEPLOY_MODE=kubernetes` so the backend can verify event payloads.
...
## E) Postgres (Managed service)
- Create DB + least-priv user; get `DATABASE_URL` (SSL if required). Smoke: `psql -c "select 1"`.

Managed PostgreSQL on DigitalOcean requires `?sslmode=require` in the connection string. The Celery tasks assume the database includes the schema deployed by Prisma migrations.

## F) Redis (Upstash or DO Managed Redis)
- Create DB; copy `rediss://` URL to `REDIS_URL`. Smoke: `ioredis set/get`.

Ensure TLS is enabled so the URL begins with `rediss://`. The backend cache and Celery share the same connection string.
...
## H) DigitalOcean secrets wiring
- Store production secrets in `infrastructure/k8s/production/secrets.yaml`. Render from a 1Password item or `.env` template.
...
Follow the rotation runbook in `docs/secrets.md` for detailed steps.
