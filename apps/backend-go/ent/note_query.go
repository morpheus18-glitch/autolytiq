...
func (_q *NoteQuery) QueryCreatedBy() *UserQuery {
...
func (_q *NoteQuery) WithCreatedBy(opts ...func(*UserQuery)) *NoteQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NoteQuery) loadCreatedBy(ctx context.Context, query *UserQuery, nodes []*Note, init func(*Note), assign func(*Note, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Note)
	for i := range nodes {
