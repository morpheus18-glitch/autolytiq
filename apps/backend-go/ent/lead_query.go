...
func (_q *LeadQuery) QueryTenant() *TenantQuery {
...
func (_q *LeadQuery) QueryCustomer() *CustomerQuery {
...
func (_q *LeadQuery) QueryAssignedTo() *UserQuery {
...
func (_q *LeadQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *LeadQuery) FirstIDX(ctx context.Context) string {
...
func (_q *LeadQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *LeadQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *LeadQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *LeadQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *LeadQuery) WithTenant(opts ...func(*TenantQuery)) *LeadQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *LeadQuery) WithCustomer(opts ...func(*CustomerQuery)) *LeadQuery {
	query := (&CustomerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LeadQuery) WithAssignedTo(opts ...func(*UserQuery)) *LeadQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LeadQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Lead, init func(*Lead), assign func(*Lead, *Tenant)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Lead)
	for i := range nodes {
		fk := nodes[i].TenantID
...
func (_q *LeadQuery) loadCustomer(ctx context.Context, query *CustomerQuery, nodes []*Lead, init func(*Lead), assign func(*Lead, *Customer)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Lead)
	for i := range nodes {
...
func (_q *LeadQuery) loadAssignedTo(ctx context.Context, query *UserQuery, nodes []*Lead, init func(*Lead), assign func(*Lead, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Lead)
	for i := range nodes {
		if nodes[i].AssignedToID == nil {
			continue
		}
		fk := *nodes[i].AssignedToID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
...
func (_q *LeadQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTenant != nil {
			_spec.Node.AddColumnOnce(lead.FieldTenantID)
		}
		if _q.withCustomer != nil {
			_spec.Node.AddColumnOnce(lead.FieldCustomerID)
		}
		if _q.withAssignedTo != nil {
			_spec.Node.AddColumnOnce(lead.FieldAssignedToID)
		}
	}
