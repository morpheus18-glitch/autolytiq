...
import { fetchBalanceSheet } from '@/lib/accountingApi';
...
export default function BalanceSheet() {
  ...
  const { data: statement, isLoading } = useQuery({
    queryKey: ['accounting', 'balance-sheet', range.startDate.toISOString(), range.endDate.toISOString()],
    queryFn: () => fetchBalanceSheet({ startDate: range.startDate.toISOString(), endDate: range.endDate.toISOString() }),
  });
  ...
  const assets = statement.sections.find((section) => section.label === 'Assets')?.total ?? 0;
  const liabilities = statement.sections.find((section) => section.label === 'Liabilities')?.total ?? 0;
  const equity = statement.sections.find((section) => section.label === 'Equity')?.total ?? 0;
  ...
  return (
    <AccountingLayout>
      <StatementHeader
        ...
        actions={<ExportButton statement="balance-sheet" startDate={range.startDate} endDate={range.endDate} />}
      />
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent className="text-2xl font-semibold text-emerald-600">{currency.format(assets)}</CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Liabilities</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold text-amber-600">{currency.format(liabilities)}</CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Equity</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold text-blue-600">{currency.format(equity)}</CardContent>
        </Card>
      </div>
