...
func (tx *Tx) init() {
	tx.AICounterOffer = NewAICounterOfferClient(tx.config)
	...
	tx.DealWorksheet = NewDealWorksheetClient(tx.config)
	tx.Lead = NewLeadClient(tx.config)
	tx.LeadScore = NewLeadScoreClient(tx.config)
	tx.Note = NewNoteClient(tx.config)
	tx.Notification = NewNotificationClient(tx.config)
	tx.Tenant = NewTenantClient(tx.config)
	tx.TradeIn = NewTradeInClient(tx.config)
...
// applies a query, for example: AICounterOffer.QueryXXX(), the query will be executed
